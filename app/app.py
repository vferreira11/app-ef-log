...
if st.button("GERAR SIMULAÇÃO"):
    ...
    for z in range(nzA):
        for y in range(nyA):
            for xi in range(nxA):
                if cnt >= nA: break
                placed_A.append((xi*largura_A, y*profundidade_A, z*altura_A,
                                 largura_A, profundidade_A, altura_A))
                cnt += 1
            if cnt >= nA: break
        if cnt >= nA: break
    ...
    for z in range(nzB):
        for y in range(nyB):
            for i in range(nxB):
                if cnt >= nB: break
                placed_B.append((x_end_A + i*largura_B, y*profundidade_B, z*altura_B,
                                 largura_B, profundidade_B, altura_B))
                cnt += 1
            if cnt >= nB: break
        if cnt >= nB: break
